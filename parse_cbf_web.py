...
def print_csv(filename,errors,headings,header,data):
	filename = re.sub("\.[a-z]+$",".csv",filename)
	print "Content-Type: application/octet-stream"
	print "Content-Disposition: attachment; filename=\"" + filename + "\""
	print ""
	for l in errors + headings :
		print l
	csvfile = [ header ]
	csvfile.extend(data)
	for l in csvfile:
...
def print_html(errors,headings,header,data):
	print "Content-Type: text/html"
	print ""
	print "<html><head><title>Converted CBF data</title></head><body>"
	for l in errors:
		print "Error: " + cgi.escape(l) + "<br>\n"
	for l in headings :
		print cgi.escape(l) + "<br>\n"
	print "<table border=\"1\" cellspacing=\"0\" cellpadding=\"3\"><tr>"
	for l in header:
		print "<th>" + cgi.escape(l) + "</th>\n"
	print "</tr>"
	for l in data:
		print "<tr>"
		for e in l:
			if e == '':
				print '<td>&nbsp;</td>'
			elif ((e[0] >= '0' and e[0] <= '9') or (e[0] == '-' and e[1] >= '0' and e[1] <= '9')):
				print "<td align=\"right\">" + cgi.escape(e) + "</td>"
			else:
...
# This is the minimal form required for the program to work
# If the styled form file does not exist, this will be presented as a fallback
request_form = """Content-Type: text/html

<!DOCTYPE html><html><head><title>Foxwell CBF Converter</title></head>
<body>
<form enctype="multipart/form-data" action="parse_cbf_web.cgi" method="post">
File to upload <input type="file" name="cbfdata"><br>
<input type="submit" name="viewhtml" value="View Table"> <input type="submit" name="downloadcsv" value="Download CSV"><br>
</form>
</body></html>
"""

if "cbfdata" in form and form["cbfdata"].value != "":
	cbf_in = form["cbfdata"].value + "<br>\n"
	cbf_fn = form["cbfdata"].filename
		
	headings,header,data,errors = parse_cbf(cbf_in)
	if "downloadcsv" in form:
		print_csv(cbf_fn,errors,headings,header,data)
	else:
		print_html(errors,headings,header,data)
else:
	try:
		fp = open("/var/www/html/parse_cbf_web.html","r")
		request_form = "Content-Type: text/html\n\n" + fp.read()
		fp.close()
	except IOError:
		pass	
